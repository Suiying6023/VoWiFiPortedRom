...
> 当前维护状态（2026-09-13）：项目聚焦注册、短信与保号。用户反馈接打电话仍失败。
> 本文保留移植实验与历史成功样本，当前使用方法以 [保号与维护](docs/maintenance.md) 为准。
...
docs/module-internals.md             模块的实现细节与设计取舍
```
...
当前接打电话仍失败；下面的拨出结果是历史实验样本。完整语音能力尚未通过当前验收。
